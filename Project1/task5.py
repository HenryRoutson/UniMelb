...
def task5():
    ...
    for review in reviews_list:
        ...
        rating_index = review["rating"]
        tweets = tweets_dict[review["news_id"]]

        rating_sums[rating_index] += n_article_tweets(tweets)
        rating_counts[rating_index] += 1

    
    x = list(range(RATINGS)) # (assumes min is 0)
    y = [rating_sums[i] / rating_counts[i] for i in range(RATINGS)] # calculate averages
